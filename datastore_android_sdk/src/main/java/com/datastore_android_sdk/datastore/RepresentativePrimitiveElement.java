...
import com.datastore_android_sdk.representation.BufferedRepresentation;
...
public class RepresentativePrimitiveElement extends PrimitiveElement {
	...
	/** The debug TAG. */
	private static final String TAG = RepresentativePrimitiveElement.class.getSimpleName();
	...
	public String toJson() {
		if (value != null && MediaType.APPLICATION_JSON.includes(value.getMediaType())) {
			try {
				...
				// Ignored
				if (Build.DEBUG) {
					Log.e(TAG, "Failed to parse representation to JSON", e);
				}
			}
		}
	...
	public String valueAsString() {
		if (isString()) {
			try {
				...
				// Ignored
				if (Build.DEBUG) {
					Log.e(TAG, "Failed to retrieve text value of representation", e);
				}
			}
		}
		return null;
