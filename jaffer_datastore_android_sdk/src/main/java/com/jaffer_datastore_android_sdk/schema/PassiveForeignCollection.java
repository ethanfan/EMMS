...
/**
 * Collection that only updates the underlying data upon request.
 * 
 * @param <T> the Java type of the component in this collection
 * @param <ID> the Java type of the identity of the component in this collection
 */
public class PassiveForeignCollection<T, ID> extends BaseForeignCollection<T, ID> implements
		...
		try {
			...
			for (String name : columns.keySet()) {
				if (attrs.isForeignCollection(name)) {
					Object foreign = columns.get(name).get(parent);
	 ...
	 * is called.
	 * 
	 * @param col a collection of elements to be added
	 * @return Returns true if the item did not already exist in the collection otherwise false.
	 */
	@Override
	 ...
	 * 
	 * @param col
	 *            the foreign collection containing collection arguments where
	...
	protected PreparedQuery<T> getPreparedQuery() throws SQLException {
		...
		if (preparedQuery == null) {
			...
			preparedQuery = qb.where().eq(getParentName(), fieldArg).prepare();
	...
	public int updateAll(Dao<T, ID> daoObject) throws SQLException {
		// Creates the data item if they do not already exist
		...
		if (daoObject != null) {
			...
			String parentColumnName = foreignFieldType == null ? null : foreignFieldType.getColumnName();
			if (parentColumnName == null) {
				parentColumnName = attributes.getColumnName(getParentName());
			}
			
			Where<T, ID> where = db.where();
			where.eq(parentColumnName, getParentId());
			if (!idList.isEmpty()) {
				where.and().notIn(attributes.getColumnName(attributes.getIdField()), idList);
			}
	...
	@Override
	public int hashCode() {
		return collection == null ? super.hashCode() : collection.hashCode();
	}
	 ...
	 * of data objects.
	 * 
	 * @return a closeable iterator that can iterate through objects in this collection
	 * @throws SQLException thrown if there was error while iterating through the collection
	 */
	@Override
